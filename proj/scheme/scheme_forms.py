...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # print('DEBUG: formals =', formals, ' body =', body)
        env.define(key, do_lambda_form(lambda_expression, env))
        return key
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while current is not nil:
        current_result = scheme_eval(current.first, env)
        if is_scheme_false(current_result):
            return False
...
def do_or_form(expressions, env):
    ...
    while current is not nil:
        current_result = scheme_eval(current.first, env)
        # print('DEBUG:', current.first, ', result = ', current_result)
        if is_scheme_true(current_result):
