...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS: # 该 first 符号是一个在 SPECIAL_FORMS 中的特殊形式
        return scheme_forms.SPECIAL_FORMS[first](rest, env)
    else:
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        # print('DEBUG: first = ', first, ', rest = ', rest)
        procedure = scheme_eval(first, env)
        # print('DEBUG: if is procedure = ', scheme_procedurep(procedure))
        scheme_params = rest.map(lambda x: scheme_eval(x, env))
        # print('DEBUG: params = ', scheme_params, ', rest = ', rest)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        try:
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            result = procedure.py_func(*params)
            return result
            # END PROBLEM 2
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        # print('DEBUG: formal =', procedure.formals)
